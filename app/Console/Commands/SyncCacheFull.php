...
class SyncCacheFull extends Command
{
    protected $signature = 'sync:cache-full 
                            {--start= : Fecha de inicio (YYYY-MM-DD)}
                            {--end= : Fecha de fin (YYYY-MM-DD)}';

    protected $description = 'Sincroniza tablas de caché de forma completa (borra y recrea datos)';

    public function handle(): int
    {
        $this->info('========================================');
        $this->info('Iniciando sincronización COMPLETA de tablas de caché...');
        $this->info('========================================');

        $startOption = $this->option('start');
        $endOption = $this->option('end');

        $start = $startOption ?? '2025-01-01';
        $end = $endOption ?? date('Y-m-d', strtotime('-1 day'));
