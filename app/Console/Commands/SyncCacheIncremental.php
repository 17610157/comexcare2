...
class SyncCacheIncremental extends Command
{
    protected $signature = 'sync:cache-incremental 
                            {--month= : Mes a sincronizar en formato YYYY-MM (por defecto el mes pasado)}';

    protected $description = 'Sincroniza tablas de caché de forma incremental (sin borrar datos existentes)';

    public function handle(): int
    {
        $this->info('========================================');
        $this->info('Iniciando sincronización INCREMENTAL de tablas de caché...');
        $this->info('========================================');

        $monthOption = $this->option('month');

        if ($monthOption) {
            $start = $monthOption.'-01';
            $end = date('Y-m-t', strtotime($start));
        } else {
            $start = date('Y-m-01', strtotime('first day of previous month'));
            $end = date('Y-m-t', strtotime('first day of previous month'));
        }
        ...
        $this->info("Período a sincronizar: {$start} hasta {$end}");
        $this->newLine();
        ...
        $results[] = $this->syncMetas($start, $end);

        $this->newLine();
        $this->info('========================================');
        $this->info('Resumen de sincronización incremental:');
        foreach ($results as $result) {
            $status = $result['success'] ? '✓' : '✗';
            $this->info("{$status} {$result['name']}: {$result['count']} registros nuevos");
        }
        $this->info('========================================');
    ...
    private function deleteExistingData(string $table, string $start, string $end, string $dateColumn = 'fecha'): void
    {
        DB::table($table)->whereBetween($dateColumn, [$start, $end])->delete();
    }

    private function syncCarteraAbonos(string $start, string $end): array
    {
        $this->info('Sincronizando cartera_abonos_cache (incremental)...');

        try {
            $this->deleteExistingData('cartera_abonos_cache', $start, $end, 'fecha');
            ...
            $count = DB::table('cartera_abonos_cache')->whereBetween('fecha', [$start, $end])->count();
    ...
    private function syncNotasCompletas(string $start, string $end): array
    {
        $this->info('Sincronizando notas_completas_cache (incremental)...');

        try {
            $this->deleteExistingData('notas_completas_cache', $start, $end, 'fecha_vta');
            ...
            $count = DB::table('notas_completas_cache')->whereBetween('fecha_vta', [$start, $end])->count();
    ...
    private function syncComprasDirecto(string $start, string $end): array
    {
        $this->info('Sincronizando compras_directo_cache (incremental)...');

        try {
            $this->deleteExistingData('compras_directo_cache', $start, $end, 'f_emision');
            ...
            $count = DB::table('compras_directo_cache')->whereBetween('f_emision', [$start, $end])->count();
    ...
    private function syncRedencionesClub(string $start, string $end): array
    {
        $this->info('Sincronizando redenciones_club_cache (incremental)...');

        try {
            $this->deleteExistingData('redenciones_club_cache', $start, $end, 'fecha');
            ...
            $count = DB::table('redenciones_club_cache')->whereBetween('fecha', [$start, $end])->count();
    ...
    private function syncVendedores(string $start, string $end): array
    {
        $this->info('Sincronizando vendedores_cache (incremental)...');

        try {
            $this->deleteExistingData('vendedores_cache', $start, $end, 'fecha');
            ...
            $count = DB::table('vendedores_cache')->whereBetween('fecha', [$start, $end])->count();
    ...
    private function syncMetas(string $start, string $end): array
    {
        $this->info('Sincronizando metas_cache (incremental)...');

        try {
            $this->deleteExistingData('metas_cache', $start, $end, 'fecha');
            ...
            $count = DB::table('metas_cache')->whereBetween('fecha', [$start, $end])->count();
